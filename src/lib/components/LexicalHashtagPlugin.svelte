<script lang="ts">
  ...
  import {
    $createHashtagNode as createTagNode,
    HashtagNode,
  } from "@lexical/hashtag";
  import { onMount } from "svelte";
  import { getEditor } from "$lib/utilities";
  import { setupLexicalTextEntity } from "$lib/utilities/setupLexicalTextEntity";

  const getHashtagRegexStringChars = (): {
    alpha: string;
    alphanumeric: string;
    hashChars: string;
  } => {
  ...
  const createHashtagNode = (textNode: TextNode): HashtagNode => {
    ...
    return createTagNode(textNode.getTextContent());
  };
  ...
  setupLexicalTextEntity<HashtagNode>(
    getHashtagMatch,
    HashtagNode,
    createHashtagNode
  );
